# ******************************************************************************
# What's the chilling-forcing significance if we use the raw data, not the
# simulated data
# 
# Author: Xiaojie Gao
# Date: 2024-06-29
...
# This is actually the original data, but to keep the code exactly the same as
# the simulation, I use `sim_` here.
sim_spec_dt <- spec_dt[, .(
    siteID = PEP_ID, 
    Date = as_date(Date), 
    Tmean, SOS, PhenoYear
...
# ~ ----------------------------------------------------------------------------
pdf("out/sos_trends2_2.pdf", width = 8)
...
hist(sim_dt_p2$SOS,
    breaks = 50, col = adjustcolor(cols[2], 0.5),
    freq = FALSE,
    border = adjustcolor("grey", 0.5),
    add = TRUE
)
...
pdf("out/fake_chill_forc2_2.pdf", width = 4, height = 4)
