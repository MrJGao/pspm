...
# Read data in
flux_dt <- fread(file.path(datadir, "Data/ARD/flux_mcd12q2_daymet_dt.csv"))
flux_dt[, Date := as_date(Date)]
# merge site meta
site_meta <- fread(file.path(datadir, "Data/ARD/sites_fluxnet2015_Tier1.csv"))
...
# HF
hf_pheno_dt <- fread(file.path(datadir, "Data/ARD/hf_grd_temperature_pheno.csv"))
hf_pheno_dt[, Date := as_date(Date)]

# HB
hb_pheno_dt <- fread(file.path(datadir, "Data/ARD/hb_grd_temperature_pheno.csv"))
